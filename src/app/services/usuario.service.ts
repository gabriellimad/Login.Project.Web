...
constructor(private httpClient: HttpClient,
  ...
  logar (usuario: IUsuario): Observable<any> {
    ...
    return this.mockUsuario(usuario).pipe(tap((resposta) => {
    if(!resposta.sucesso) return;
    localStorage.setItem('token', btoa(JSON.stringify('TokenQueViriaViaAPI')));
    localStorage.setItem('usuario', btoa(JSON.stringify(usuario)));
    this.router.navigate(['']);
    }));
  }

  private mockUsuario (usuario: IUsuario): Observable<any> {
    var retornoMock: any = [];
    if (usuario.email === "gabriel@gmail.com" && usuario.senha === "123") {
      retornoMock.sucesso = true;
      retornoMock.usuario = usuario;
      retornoMock.token = "TokenQueViriaViaAPI";
